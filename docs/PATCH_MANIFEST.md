...
| `login-no-redirect-after-auth` | `app/public/src/pages/component/auth/login.tsx` | Returns `false` from FirebaseUI `signInSuccessWithAuthResult` so it does not re-navigate to `signInSuccessUrl` after a successful redirect-flow sign-in. Prevents the same-URL reload that interrupts Firebase's IndexedDB auth-state persist and bounces the user back to login. |
